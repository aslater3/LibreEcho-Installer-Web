...
test('flash singleDownload sends one whole raw image despite a smaller reported max-download-size', async () => {
  const size = 0x06d0d60c;
  const image = new Uint8Array(size);
  image[0] = 0x41;
  image[size - 1] = 0x54;
  const device = makeDevice({
    vars: { 'max-download-size': '0x6d00000' },
    offeredChunk: size,
  });
  const transport = new FakeTransport({ respond: device.respond });
  // Large USB writes are packetised; avoid a second whole-image copy in the fake.
  transport.write = async (bytes) => {
    transport.writes.push(bytes);
    const reply = device.respond(dec(bytes.subarray(0, Math.min(bytes.length, 32))), transport, bytes);
    if (reply) transport.push(...reply);
  };
  const client = new FastbootClient(transport, { timeout: 500 });
  assert.equal(await client.getVar('max-download-size'), '0x6d00000');
  const progress = [];
  const result = await client.flash('brick', image, {
    singleDownload: true,
    onProgress: (sent, total) => progress.push([sent, total]),
  });
  assert.deepEqual(result, { bytes: size, info: ['Sending sparse image...'] });
  assert.deepEqual(device.commands, ['getvar:max-download-size', 'download:06d0d60c', 'flash:brick']);
  assert.deepEqual(progress, [[size, size]]);
  assert.equal(device.state.payloads.length, 1);
  assert.equal(device.state.payloads[0][0], 0x41);
  assert.equal(device.state.payloads[0][size - 1], 0x54);
});

test('flash singleDownload rejects a reduced DATA window before payload or flash', async () => {
  const { device, client } = setup({ offeredChunk: 0x800 });
  await assert.rejects(client.flash('brick', new Uint8Array(0x1000), { singleDownload: true }), (err) => {
    assert.ok(err instanceof FastbootProtocolError);
    assert.equal(err.command, 'download:00001000');
    assert.match(err.message, /expected DATA window of 4096 bytes.*2048 bytes/);
    return true;
  });
  assert.deepEqual(device.commands, ['download:00001000']);
  assert.deepEqual(device.payloads, []);
});

test('flash singleDownload propagates device refusals without attempting flash', async () => {
  for (const phase of ['download', 'payload']) {
    const { device, client } = setup({
      onCommand: (command) => (phase === 'download' && command.startsWith('download:')
        ? [failFrame('image too large')] : undefined),
      onPayload: () => (phase === 'payload' ? [failFrame('buffer rejected')] : undefined),
    });
    await assert.rejects(client.flash('brick', new Uint8Array(0x100), { singleDownload: true }), (err) => {
      assert.ok(err instanceof FastbootFailError);
      assert.equal(err.reason, phase === 'download' ? 'image too large' : 'buffer rejected');
      return true;
    });
    assert.deepEqual(device.commands, ['download:00000100']);
  }
});

test('flash singleDownload bounds stalled payload writes and never flashes', async () => {
  const device = makeDevice();
  const transport = new FakeTransport({ respond: device.respond });
  const write = transport.write.bind(transport);
  transport.write = (bytes) => device.state.pendingDownload !== null
    ? new Promise(() => {}) : write(bytes);
  const client = new FastbootClient(transport, { timeout: 100 });
  await assert.rejects(client.flash('brick', new Uint8Array(16), {
    singleDownload: true, writeTimeoutMs: 5,
  }), (err) => err instanceof FastbootTimeoutError && err.command === 'download:00000010');
  assert.deepEqual(device.commands, ['download:00000010']);
});

test('flash singleDownload refuses invalid opt-in inputs without touching USB', async () => {
  const { device, client } = setup();
  await assert.rejects(client.flash('boot_a', new Uint8Array(4), { singleDownload: true }), /requires a brick image/);
  await assert.rejects(client.flash('brick', null, { singleDownload: true }), /requires a brick image/);
  await assert.rejects(client.flash('brick', new Uint8Array(4), { singleDownload: true, chunkSize: 2 }), /no chunkSize/);
  await assert.rejects(client.flash('brick', new Uint8Array(0), { singleDownload: true }), /fastbrick image must be/);
  await assert.rejects(client.flash('brick', new Uint8Array(4), { singleDownload: 'true' }), /singleDownload must be a boolean/);
  await assert.rejects(client.flash('brick', new Uint8Array(4), { singleDownload: true, writeTimeoutMs: 0 }), /writeTimeoutMs/);
  assert.deepEqual(device.commands, []);
});

test('flash singleDownload requires an OKAY acknowledgement before flashing', async () => {
  const { device, client } = setup({ onPayload: () => [] });
  await assert.rejects(client.flash('brick', new Uint8Array(16), { singleDownload: true }), (err) => {
    assert.ok(err instanceof FastbootTimeoutError);
    assert.equal(err.command, 'download:00000010');
    return true;
  });
  assert.deepEqual(device.commands, ['download:00000010']);
  assert.equal(device.payloads.length, 1);
});

test('flash singleDownload refuses an image beyond its host size cap before USB', async () => {
  const { device, client } = setup();
  await assert.rejects(client.flash('brick', new Uint8Array(0x08000001), { singleDownload: true }), /fastbrick image must be/);
  assert.deepEqual(device.commands, []);
});
