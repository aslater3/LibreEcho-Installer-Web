...
export class FastbootClient {
   ...
   * @param {{onProgress?: ((sent: number, total: number) => void)|null, chunkSize?: number, onInfo?: ((line: string) => void)|null, singleDownload?: boolean, writeTimeoutMs?: number}} [options]
   *   `singleDownload` is an explicit whole-image path for brick only. It ignores
   *   max-download-size, requires the full DATA window and never retries/chunks.
   ...
   */
  async flash(partition, bytes = null, { onProgress = null, chunkSize = 0, onInfo = null, singleDownload = false, writeTimeoutMs = 900000 } = {}) {
    const report = onInfo ?? this.onInfo ?? null;
    let downloaded = 0;
    if (typeof singleDownload !== 'boolean') {
      throw new TypeError('fastboot: singleDownload must be a boolean');
    }
    if (singleDownload) {
      if (partition !== 'brick' || bytes === null || bytes === undefined || chunkSize) {
        throw new FastbootError('fastboot: singleDownload requires a brick image and no chunkSize');
      }
      downloaded = await this._downloadWholeImage(bytes, { onProgress, onInfo: report, writeTimeoutMs });
  ...
  /**
   * Whole-image download for an explicitly selected fastbrick. Unlike
   * download(), this bypasses the reported max-download-size and never narrows
   * or retries a DATA window. Flash is handled by flash() only after OKAY.
   * The write deadline cannot cancel an in-flight USB transfer: disconnect and
   * reconnect after a timeout rather than reusing the transport.
   * @param {Uint8Array|ArrayBuffer|ArrayBufferView|number[]|Blob} bytes
   * @param {{onProgress?: ((sent: number, total: number) => void)|null, onInfo?: ((line: string) => void)|null, writeTimeoutMs: number}} options
   * @returns {Promise<number>}
   */
  async _downloadWholeImage(bytes, { onProgress, onInfo, writeTimeoutMs }) {
    if (!Number.isInteger(writeTimeoutMs) || writeTimeoutMs <= 0 || writeTimeoutMs > 900000) {
      throw new RangeError('fastboot: fastbrick writeTimeoutMs must be between 1 and 900000');
    }
    const data = await toBytes(bytes, 'fastbrick image');
    if (data.length <= 0 || data.length > 0x08000000) {
      throw new RangeError('fastboot: fastbrick image must be between 1 and 134217728 bytes');
    }
    const command = `download:${toHex8(data.length)}`;
    const res = await this._command(command);
    if (res.status === STATUS_FAIL) throw new FastbootFailError(command, this._reason(res));
    if (res.status !== STATUS_DATA || res.dataLength !== data.length) {
      throw new FastbootProtocolError(command,
        `expected DATA window of ${data.length} bytes, received ${res.status} (${res.dataLength} bytes)`);
    }
    let timer;
    try {
      await Promise.race([
        ...
        new Promise((_, reject) => {
          timer = setTimeout(() => reject(new FastbootTimeoutError(command, writeTimeoutMs, 'payload write')), writeTimeoutMs);
        }),
      ]);
    } catch (cause) {
      if (cause instanceof FastbootTimeoutError) throw cause;
      throw new FastbootError(`fastboot: transport failed to send the payload for "${command}": ${messageOf(cause)}`, { command, cause });
    } finally {
      clearTimeout(timer);
    }
    ...
    for (const line of this._lines(ack)) if (onInfo) onInfo(line);
