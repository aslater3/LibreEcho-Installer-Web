  ...
  async push(remotePath, data, options = {}) {
    ...
    try {
      await this._writeToStream(
        stream,
        packSync('SEND', encodeUtf8(`${remotePath},${mode}\0`)),
        deadline,
        `push(${remotePath}) SEND`
      );
      await this._syncExpectOkay(stream, deadline, `SEND ${remotePath}`);
      ...
      await this._writeToStream(stream, packSync('DONE', u32Bytes(mtime)), deadline, `push(${remotePath}) DONE`);
