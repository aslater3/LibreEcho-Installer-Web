...
class FakeAdbd {
  ...
  _onSyncBytes(stream, payload) {
    ...
    while (stream.buf.length >= 8) {
      ...
      // AOSP sync: DONE's second word is mtime, not a payload length.
      const payloadLength = id === 'DONE' ? 0 : len;
      if (stream.buf.length < 8 + payloadLength) break;
      const data = stream.buf.slice(8, 8 + payloadLength);
      stream.buf = stream.buf.subarray(8 + payloadLength).slice();
  ...
  _handleSync(stream, id, len, data) {
    switch (id) {
      case 'SEND': {
        ...
        stream.file = { path, mode, chunks: [], size: 0, mtime: 0, errno };
        return; // SEND has no sync reply; adbd reports success/failure after DONE.
      }
      case 'DATA': {
      ...
      case 'DONE': {
        ...
        stream.file.mtime = len;
        if (stream.file.errno) {
          this.stats.syncFailSent += 1;
          this._syncFail(stream, stream.file.errno);
          stream.file = null;
          return;
        }
...
test('push(): spec peer waits until DONE, with mtime in the DONE header', async () => {
  const { adbd, client } = makeClient({ client: { pushTimeout: 200 } });
  await client.connect();
  const path = '/data/local/tmp/empty.bin';
  const mtime = 0x12345678;
  const result = await client.push(path, EMPTY, { mode: 0o600, mtime });

  assert.equal(result.bytes, 0);
  assert.equal(adbd.stats.syncOkaySent, 1, 'sync peer replies only after DONE, not after SEND');
  const syncFrames = adbd.messages.filter((m) => m.command === A_WRTE).map((m) => m.payload);
  assert.deepEqual(syncFrames.map(text4), ['SEND', 'DONE', 'QUIT']);
  assert.deepEqual(syncFrames[0], cat(cat(enc.encode('SEND'), u32le(enc.encode(`${path},384`).length)), enc.encode(`${path},384`)),
    'SEND is the exact path,decimal-mode string, without a NUL');
  assert.deepEqual(syncFrames[1], cat(enc.encode('DONE'), u32le(mtime)),
    'DONE is exactly 8 bytes: id and little-endian mtime, not a length-4 payload');
  assert.deepEqual(adbd.files.get(path), { mode: 0o600, size: 0, mtime, data: EMPTY });
  assert.deepEqual(adbd.errors, []);
});
...
test('push(): multi-chunk payload with progress and per-WRTE checksum verification', async () => {
  ...
  assert.equal(adbd.stats.syncOkaySent, 1, 'sync OKAY is sent only after DONE');
...
test('push(): a rejected SEND is reported after DONE with the device errno', async () => {
  const { adbd, client } = makeClient();
  await client.connect();
  ...
  await assert.rejects(client.push('/system/readonly.txt', new Uint8Array(32)), (err) => {
    ...
    assert.match(err.message, /DONE \/system\/readonly\.txt/);
    return true;
  });
  assert.deepEqual(adbd.messages.filter((m) => m.command === A_WRTE).map((m) => text4(m.payload)),
    ['SEND', 'DATA', 'DONE'], 'the error arrives only after DONE');
